    ...
    def middleware(request):
        ...
        if check_visitor_log() == "True":  # If logging is enabled
            if path not in IGNORED_PATH:  # If path is not in ignored paths
                visitor = create_visitor(ip_address=ip, user_agent=user_agent, path=path, data=data)
                if (
                    path.startswith(f"/{ADMIN_URL}/") and request.user.is_authenticated
                ):  # If path is admin path and user is authenticated
